...
async function main() {
  ...
  const initialPosX = 0; //Math.ceil(Math.random()*400);
  const initialPosY = 350; //Math.ceil(Math.random()*400);
  const initialAngle = 0; //Math.ceil(Math.random()*360);
  const batship = StarShip.create(galaxy, './assets/spaceship/batship.png', 'small batship', initialPosX, initialPosY, initialAngle)
  batship.play()
  addKeyEvent(batship)
